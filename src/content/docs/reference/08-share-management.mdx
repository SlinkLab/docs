...
head:
  ...
  - tag: link
    attrs:
      rel: canonical
      href: https://docs.slinkapp.io/reference/08-share-management/
---
...
Each row exposes inline controls that map directly to the [Shares API](/api/06-shares):
...
If you create a share programmatically via the [API](/api/03-images#create-or-get-share-for-an-image), it starts **unpublished** so you can set a password or expiration before exposing the link, then publish it explicitly via [`PUT /api/share/{id}/publish`](/api/06-shares#publish-share).
