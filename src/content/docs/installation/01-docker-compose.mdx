---
title: Docker Compose Installation
description: Install Slink using Docker Compose. Step-by-step container deployment instructions, with storage options for local, SMB, and S3.
head:
  - tag: meta
    attrs:
      name: keywords
      content: slink installation, docker compose, slink docker setup, self-hosted image sharing, container deployment, slink storage configuration, local storage, smb storage, s3 storage, docker image hosting
  - tag: meta
    attrs:
      property: og:type
      content: article
  - tag: meta
    attrs:
      name: robots
      content: index, follow, max-image-preview:large
  - tag: link
    attrs:
      rel: canonical
      href: https://docs.slinkapp.io/installation/01-docker-compose/
  - tag: script
    attrs:
      type: application/ld+json
    content: |
      {
        "@context": "https://schema.org",
        "@type": "HowTo",
        "name": "Install Slink using Docker Compose",
        "description": "Step-by-step instructions to install Slink using Docker Compose with configuration examples for local, SMB, and S3 storage.",
        "url": "https://docs.slinkapp.io/installation/01-docker-compose/",
        "image": "https://docs.slinkapp.io/slink.png",
        "totalTime": "PT10M",
        "supply": [
          {
            "@type": "HowToSupply",
            "name": "Docker"
          },
          {
            "@type": "HowToSupply", 
            "name": "Docker Compose"
          }
        ],
        "step": [
          {
            "@type": "HowToStep",
            "name": "Create directory and navigate to it",
            "text": "Create a new directory for Slink and navigate to it",
            "instruction": "mkdir slink-app && cd slink-app"
          },
          {
            "@type": "HowToStep",
            "name": "Create docker-compose.yml file",
            "text": "Create a docker-compose.yml file with the Slink service configuration"
          },
          {
            "@type": "HowToStep",
            "name": "Start the application",
            "text": "Run docker-compose up -d to start the Slink application",
            "instruction": "docker-compose up -d"
          },
          {
            "@type": "HowToStep",
            "name": "Activate user account (optional)",
            "text": "If user approval is required, activate your user account using the CLI command"
          },
          {
            "@type": "HowToStep",
            "name": "Grant admin access (optional)",
            "text": "Grant yourself admin access by running the user role assignment CLI command"
          },
          {
            "@type": "HowToStep",
            "name": "Create account and login",
            "text": "Navigate to the signup page in your browser to create a new account and start using Slink"
          }
        ]
      }
---

import { Steps, Aside } from '@astrojs/starlight/components';

## Prerequisites

Before you begin, make sure you have [Docker](https://www.docker.com/) and [Docker Compose](https://docs.docker.com/compose/) installed on your system.

## Installation

<Aside type="caution" title="Important">
  Make sure to replace `https://your-domain.com` with your actual domain name.
  Also, ensure that the `TZ` environment variable is set to your timezone.
</Aside>

<Aside type="tip" title="Security Recommendation">
  For enhanced security, it's recommended to run the container as a non-root user. See the [Non-Root Container User](/security/02-non-root-container-user) guide for detailed instructions. Uncomment the `user:` line in the configuration below to enable this.
</Aside>

<Aside type="tip" title="SSL/HTTPS Recommendation">
  For production deployments, it's highly recommended to enable SSL by setting `REQUIRE_SSL=true` in your environment variables. This setting affects authentication and ensures secure communication.
</Aside>

<Steps>

1. Create a new directory for Slink and navigate to it:

   ```bash
   mkdir slink-app && cd slink-app
   ```

2. Create a `docker-compose.yml` file with the following content:

   ```yaml
   services:
     slink:
       image: anirdev/slink:latest
       container_name: slink
       # This container runs as root user by default. Uncomment the "user: slink" line below to run as non-root user.
       # See https://docs.slinkapp.io/security/02-non-root-container-user/ for additional details.
       # user: slink
       environment:
         # Your timezone
         - TZ=UTC

         # Your application hostname (Required for cookies)
         - ORIGIN=https://your-domain.com

         # Require user approval before they can upload images
         - USER_APPROVAL_REQUIRED=true

         # User password requirements
         - USER_PASSWORD_MIN_LENGTH=8
         - USER_PASSWORD_REQUIREMENTS=15 # bitmask of requirements

         # Automatically create first admin user (optional - remove after initial setup)
         # - ADMIN_USERNAME=admin
         # - ADMIN_EMAIL=admin@your-domain.com
         # - ADMIN_PASSWORD=YourSecurePassword123

         # Maximum image size allowed to be uploaded (up to 1000M)
         - IMAGE_MAX_SIZE=15M

         # Image processing settings
         - IMAGE_STRIP_EXIF_METADATA=true
         - IMAGE_COMPRESSION_QUALITY=80

         # Storage provider to use (may require additional configuration variables for different providers, see below)
         - STORAGE_PROVIDER=local
       volumes:
         # Persist the database
         - ./slink/var/data:/app/var/data
         # Persist the uploaded images
         - ./slink/images:/app/slink/images
       ports:
         # Expose the application on port 3000
         - '3000:3000'
   ```

3. Run the following command to start the Slink application:

   ```bash
    docker-compose up -d
   ```

4. Set up your first admin user following the [First User Setup](/getting-started/03-first-user-setup) guide.

   <Aside type="tip">
     You can configure the admin user automatically during initial deployment by uncommenting and setting the `ADMIN_EMAIL` environment variable in step 2.
   </Aside>

5. Open your browser and navigate to [https://your-domain.com/profile/signup](https://your-domain.com/profile/signup) to create a new account (or login if you used automatic admin creation).

6. Login to the Slink application and start uploading images.

</Steps>

## Storage Configuration

The configuration above uses local storage, which is the default. To store images on a network share (SMB/CIFS) or in Amazon S3 or an S3-compatible service, set `STORAGE_PROVIDER` and its related variables. See the [Storage Providers](/configuration/02-storage-provider) reference for full setup and examples.

<Aside type="tip">
  With SMB or S3, images are stored remotely, so only the database volume (`./slink/var/data`) needs to be persisted. The `./slink/images` volume can be omitted.
</Aside>
