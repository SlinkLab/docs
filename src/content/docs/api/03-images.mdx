...
GET /image/123e4567-e89b-12d3-a456-426614174000.jpg?width=800&height=600
GET /image/123e4567-e89b-12d3-a456-426614174000.webp?width=300&crop=true
```
...
### Create or Get Share for an Image

Returns a share for the given image (with optional transformation parameters). If a share already exists for that exact target, the existing one is returned; otherwise a new one is created.

**Endpoint:** `GET /api/image/{id}/share`
...
- `crop` - Enable cropping: `true` or `false` (optional, default: false)
- `format` - Output format extension, e.g. `jpg`, `webp` (optional)
- `filter` - Named filter to apply (optional)

**Response (HTTP 200 if reused, HTTP 201 if newly created):**
```json
{
  "shareId": "share-uuid",
...
Newly created image shares are **unpublished by default** at the API level. Use [`PUT /api/share/{id}/publish`](/api/06-shares#publish-share) to make the link reachable.

In the Slink UI, the **Copy Link** action on an image performs both steps in sequence (create-or-fetch the share, then publish it), so a share is effectively considered published the moment its link is copied from the UI.
...
```
# Resize to 800x600
/image/uuid.jpg?width=800&height=600

# Resize with cropping enabled
/image/uuid.jpg?width=400&height=400&crop=true

# Just resize width, maintain aspect ratio
/image/uuid.jpg?width=800
```
...
```json
{
  "error": {
    ...
    "violations": [
      {
        "property": "image",
        "message": "The mime type of the file is invalid (\"application/pdf\"). Allowed mime types are \"image/bmp\", \"image/png\", \"image/jpeg\", \"image/gif\", \"image/webp\", \"image/svg+xml\", \"image/x-icon\", \"image/avif\", \"image/heic\", \"image/tiff\"."
      }
    ]
  }
}
